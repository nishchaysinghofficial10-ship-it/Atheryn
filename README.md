...
Start here if ATHERYN is new to you. The short version is simple: each conclusion has to lead back to a recorded experiment.
...
| | benchmark & report | ask a model | ATHERYN |
|---|---:|---:|---:|
| Experiments | 1 | 0 | 6 |
| Conclusions | 4 | 2 | 2 |
...
notice. ATHERYN spent six experiments instead of one, produced *fewer*
...
```bash
unzip atheryn-v2.0.0.zip && cd atheryn-project     # or: git clone …
python -m unittest discover -s tests             # 261 tests, ~90 s, no install

python -m atheryn init "Which sorting strategy wins under which input regime?" \
    --dir runs/demo --profile fast
python -m atheryn run    --dir runs/demo
python -m atheryn status --dir runs/demo
python -m atheryn report --dir runs/demo          # the research dossier
```
...
```bash
python -m atheryn init "…" --dir runs/g --domain graphbench --profile graph_fast
```
...
textbook-plausible hypotheses ATHERYN had proposed itself.
...
python -m atheryn replay --dir examples/flagship_run --exp <exp_id>
```
