# ATHERYN Technical Specification v0.1

This specification records the intended boundaries. The tests and stored artifacts show which parts are implemented today.
...
This page is the formal contract for v0.1 and the foundation every later
...
ATHERYN is not "an AI that researches anything." It is a **persistent
...
(`atheryn/models.py::EpistemicStatus`):
...
| `experimental_result` | Produced by ATHERYN's own experiments |
...
All records are dataclasses in `atheryn/models.py`, serialized as JSON.
...
`atheryn/state.py`. Per-project on-disk layout:
...
Budget exhaustion at any point short-circuits to 5→6: ATHERYN reports what it
has rather than pretending completeness.

## 7. Research domain interface

`atheryn/domains/base.py::ResearchDomain` — the core is domain-agnostic:
...
A domain that emits new hypotheses from `analyze()` gives ATHERYN
...
value-per-cost, making ATHERYN an active allocator rather than a crawler.
...
- `atheryn status` — mission-control box (counts, budgets, current
  investigation).

## 11. Evaluation methodology (Phase 8 preview)

Metrics ATHERYN is built to expose from day one: research efficiency
...
`run.py` re-runnable standalone). ORB (ATHERYN Research Benchmark): a suite
of bounded, experimentally checkable problems with fixed budgets, comparing
ATHERYN against one-shot answers and fixed pipelines. Deferred to Phase 8;
...
pause/resume/cancel/recover-lock); `atheryn verify` coverage of autonomy state;
a fixture-only demonstration in `examples/autonomy_demo/`; 40 autonomy tests
(226 total).

Still intentionally absent: any daemon or background service, multi-agent
