"""Adversarial checks for the structured proposal boundary.
...
between any provider and the research engine:
    ...
    provider text -> parse -> schema -> domain/policy -> audit -> ATHERYN decides
...
from atheryn import brain as B                                  # noqa: E402
from atheryn import proposals as P                              # noqa: E402
...
from atheryn.models import EpistemicStatus, HypothesisStatus    # noqa: E402
...
class TestProposalPipeline(unittest.TestCase):
    ...
    def test_experiment_proposal_runs_under_atheryn_controlled_parameters(self):
        exp_proposal = {
            "proposal_type": "experiment",
        ...
        for rec in st.experiments.values():
            if rec.design.get("round") == 5:
                # ATHERYN owns determinism and limits, not the provider
    ...
    def test_dossier_reports_the_proposal_ledger_truthfully(self):
        ...
        ctl.run()
        from atheryn.report import write_reports
...
class TestProviderReliability(unittest.TestCase):
    ...
    def test_raw_audit_when_explicitly_enabled_is_redacted(self):
        os.environ["ATHERYN_LLM_AUDIT_RAW"] = "1"
        try:
            env = json.dumps({"content": [{"type": "text", "text": "[]"}]})
            brain = B.AnthropicBrain(_transport=lambda _b: env,
                                     audit_dir=str(self.tmp))
            ...
            os.environ.pop("ATHERYN_LLM_AUDIT_RAW", None)
