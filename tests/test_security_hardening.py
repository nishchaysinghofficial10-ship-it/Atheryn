"""Attack-oriented regressions for newer ATHERYN surfaces.
...
from atheryn import autonomy as A                              # noqa: E402
from atheryn import sandbox                                    # noqa: E402
from atheryn.budget import Budget                              # noqa: E402
from atheryn.cli import PROFILES, main as cli_main             # noqa: E402
...
from atheryn.scheduler import Scheduler                        # noqa: E402
...
class TestSecondDomainExecutionSafety(Base):
    ...
    def test_a_mission_config_does_not_leak_into_other_missions(self):
        ...
        from atheryn.cli import PROFILES as _PROFILES
...
class TestPublicArtifactClaims(Base):
    ...
    def test_dossier_reports_the_real_version(self):
        """The dossier header was hardcoded to "ATHERYN v1.0" for five releases —
        ...
        self.assertIn(f"v{atheryn.__version__}", header)
        self.assertNotIn("v1.0 ", header)

    def test_cli_survives_a_closed_pipe(self):
        """`atheryn report | head` printed a BrokenPipeError traceback."""
        import subprocess
        st = mission(self.tmp, "v2")
        ResearchController(st, get_domain("graphbench")).run()
        repo = Path(__file__).resolve().parents[1]
        proc = subprocess.run(
            f'python3 -m atheryn report --dir "{st.root}" | head -3',
