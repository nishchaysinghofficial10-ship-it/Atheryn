"""Compare a fresh experiment run with its stored result.

`atheryn replay` re-executes a stored experiment from its recorded code and
...
What a replay actually guarantees, in three tiers:
  ...
  STATISTICAL REPRODUCIBILITY — reported always, asserted only with --strict.
      ...
      as overturned only under the conservative rule in `atheryn.stats`
