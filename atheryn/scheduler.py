"""Choose and dispatch one restart-safe autonomy action at a time.
...
ends, nothing of ATHERYN is left executing.
"""
...
class Scheduler:
    ...
    def recover(self, state) -> list[str]:
        ...
        ATHERYN does not guess whether a claimed action completed. The item is
        ...
        for item in self.store.by_status(A.CLAIMED):
            ...
            self.store.record_decision({
                ...
                "outcome": "interrupted",
                "detail": "claimed but not completed; ATHERYN will not assume "
                          "success or re-run it automatically",
                "operator_action": "inspect with 'atheryn autonomy status', then "
