"""User-space limits applied to generated experiment processes.
...
this environment does not have. Mitigation: ATHERYN only ever executes code
...
def make_preexec(timeout_s: float, policy: dict | None = None):
    if resource is None or os.name != "posix":   # pragma: no cover
        raise RuntimeError(
            "ATHERYN requires POSIX (rlimits + os.setsid) to confine experiment "
