"""Narrow HTTPS retrieval with policy checks at every hop.

ATHERYN fetches documents. It does not browse, crawl, execute, or obey them.
...
USER_AGENT = ("ATHERYN-research/1.4 (+https://example.invalid/atheryn; "
...
def canonical_url(url: str) -> str:
    ...
    returns, so it is not ATHERYN's to normalise.
    """
...
def bounded_decompress(data: bytes, encoding: str, limit: int) -> bytes:
    ...
    if not dec.eof:
        ...
        # correspond to the text ATHERYN kept, so it is an error instead.
