"""Sorting benchmarks, ATHERYN's first concrete research domain.

Chosen deliberately as the v0.1 domain because success is objectively
measurable: ATHERYN can generate real code, execute it in a sandbox, measure
...
def hybrid_sort(a):
    """ATHERYN-generated candidate: merge sort with an insertion-sort cutoff.
...
def make_hybrid(cutoff):
    ...
    (used by ATHERYN's cutoff parameter-sweep experiments)."""
    def _hybrid(a):
        a = list(a)
...
RUNNER_TEMPLATE = '''"""Auto-generated by ATHERYN (algobench domain). Self-contained experiment.
...
class AlgoBenchDomain(ResearchDomain):
    ...
    def write_runner(self, design: dict, exp_dir: Path) -> Path:
        ...
        from .. import __version__ as atheryn_version
        runner_config = dict(design, atheryn_version=atheryn_version)
    ...
    def _eval_prediction(self, check, rows, design, n_top, winners):
        ...
        `atheryn.stats`: a difference that the trial count and spread cannot
        resolve is reported as INCONCLUSIVE rather than being called a win or a
        loss. This is what keeps ATHERYN from turning scheduler noise into a
        finding.
        """
        t = check["type"]
