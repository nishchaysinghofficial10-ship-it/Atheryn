"""Human-readable status, timeline, and dossier rendering.
...
def status_box(state) -> str:
    ...
    lines = [
        "╔" + "═" * (W - 2) + "╗",
        row("PROJECT ATHERYN".center(W - 4)),
        sep,
...
def render_timeline(state) -> str:
    lines = ["# ATHERYN Research Timeline", "",
...
def render_dossier(state, domain) -> str:
    L: list[str] = []
    add = L.append
    add("# ATHERYN Research Dossier")
    add("")
    from . import __version__ as _atheryn_version
    add(f"Generated: {time.strftime('%Y-%m-%d %H:%M:%S')}  |  ATHERYN "
        f"v{_atheryn_version}  |  domain: `{domain.name}`")
    add("")
    ...
    if gen:
        for h in gen:
            add(f"- ATHERYN synthesized a new candidate from round-1 evidence: **{h.statement}** "
    ...
    if not audit:
        ...
        add("")
        add("| Proposal | Type | Verdict | What ATHERYN did with it |")
        add("|---|---|---|---|")
        for a in acc:
    ...
    add("")
    add("- Recreate this mission: `python -m atheryn init \"<question>\" --dir <new_dir> "
        "--profile <profile>` then `python -m atheryn run --dir <new_dir>`")
    add("- Replay any experiment from stored metadata: "
        "`python -m atheryn replay --dir <this_dir> --exp <exp_id>`")
    add("- Verify state consistency: `python -m atheryn verify --dir <this_dir>`")
...
<title>ATHERYN mission control</title><style>
