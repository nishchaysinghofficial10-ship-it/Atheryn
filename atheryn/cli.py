"""The command-line interface used to create, run, and inspect missions.

    python -m atheryn init "QUESTION" --dir runs/demo [--brain mock|anthropic|none]
    python -m atheryn run --dir runs/demo [--steps N]
    python -m atheryn status | report | timeline | html | verify --dir runs/demo
    python -m atheryn replay --dir runs/demo --exp exp_xxxx [--tolerance 0.5]
    python -m atheryn ingest --dir runs/demo --file notes.md
    python -m atheryn cancel --dir runs/demo
"""
...
def _quiet_broken_pipe() -> None:
    """`atheryn report | head` must not print a traceback.
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(
        prog="atheryn",
        description="ATHERYN — persistent computational research engine (v1.0)")
    ...
    if args.cmd == "init":
        ...
        print(status_box(st))
        print("\nNext:  python -m atheryn run --dir", root)
        return 0
    ...
    try:
        ...
        print(f"NO PROJECT: {e}\nCreate one with: python -m atheryn init "
    ...
    if args.cmd == "run":
        ...
        if st.meta.get("phase") == lc.COMPLETED:
            ...
            print("\nPaused. Resume with: python -m atheryn run --dir", root)
        return 0
...
def _autonomy(args, root, st, domain) -> int:
    ...
    if cmd == "pause":
        ...
        print("Pause requested. The current tick finishes and checkpoints; no "
              "further action starts.\nResume with: python -m atheryn autonomy "
              f"resume --dir {root}")
        return 0
    ...
    if cmd == "run":
        ...
        if out.get("detail"):
            print(f"  {out['detail']}")
        print(f"\nInspect with: python -m atheryn autonomy status --dir {root}")
        return 0
