"""The validated boundary between provider text and the research engine.

This module is the *stable internal interface* between any language model and
ATHERYN's research engine. The engine never sees a provider response; it sees
...
Four proposal types are supported. Each carries enough structure for ATHERYN to
...
# `predicted_measurement` is the only bridge from prose to something ATHERYN can
...
def parse_provider_json(text: str) -> list:
    ...
    meaning change. Anything else that is not valid JSON is rejected; ATHERYN
    never repairs a malformed proposal, because a repaired proposal is a
    proposal ATHERYN wrote.
    """
...
def validate_policy(raw: dict, ptype: str, domain, state,
    ...
    if ptype == HYPOTHESIS and not measurement:
        problems.append("hypothesis proposals must carry a "
                        "predicted_measurement ATHERYN can check")
    if measurement:
    ...
    if design:
        ...
        # The same policy gate that guards ATHERYN's own designs.
...
# audit log
class ProposalAudit:
    """Append-only audit of every proposal ATHERYN was offered.

    Accepted and rejected alike, with the reason and the proposal body, so a
    reviewer can reconstruct what the model asked for and why ATHERYN refused.
