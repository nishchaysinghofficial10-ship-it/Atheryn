"""The research loop and the decisions that move it forward.
...
checkpoints the entire research state. Interrupt it at any point; `atheryn
...
class ResearchController:
    ...
    def _merge_brain_proposals(self) -> None:
        ...
        Pipeline (atheryn/proposals.py owns every gate):
            provider response -> strict JSON parse -> schema validation
            -> domain/policy validation -> append-only audit log -> ATHERYN decides
        ...
        Accepted proposals get NO evidential privilege:
          ...
          * experiment designs are stored as *candidate* designs whose
            ATHERYN-controlled fields (seed, timeout, round, hypothesis ids) are
            filled in by ATHERYN and which still pass the sandbox policy gate at
            execution time;
    ...
    def _upgrade_legacy(p):
        ...
        with an accurate reason instead of ATHERYN crashing (or, worse, quietly
    ...
    def _admit_proposal(self, p: prop.Proposal) -> str:
        ...
        if p.proposal_type == prop.EXPERIMENT:
            ...
            st.log_event("experiment_proposal",
                         ...
                         f"ATHERYN sets seed, timeout and scope at execution time")
    ...
    def _llm_candidate_design(self, primary):
        ...
        coverage are set by ATHERYN, and the result still faces the sandbox
        ...
        design = {
            ...
            "seed": cfg.get("seed", 1234),          # ATHERYN owns determinism
            "timeout_s": cfg.get("timeout_s", 600),  # ATHERYN owns the limit
            "hypothesis_ids": [primary.id],
        }
        st.log_event("experiment_proposal_used",
                     f"candidate design {cand['proposal_id']} instantiated for "
                     f"{primary.id} with ATHERYN-controlled seed/timeout")
        return design
