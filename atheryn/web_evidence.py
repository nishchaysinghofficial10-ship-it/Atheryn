"""Turn policy-checked web material into traceable speculation.

    approved URL
      → policy-checked retrieval        (atheryn/retrieval.py)
...
What this module refuses to do, by construction:
  ...
  * promote anything retrieved to FACT, or create an `Evidence` item from it —
    in the algorithms domain, findings come from ATHERYN's own experiments;
...
# ATHERYN's own measurements, whatever it is.
RELIABILITY_RULES = [
...
def ingest_url(state, url: str, provider: EvidenceProvider,
    ...
    src = Source(
        ...
        license_note="not verified by ATHERYN; check the source's own terms",
    ...
    for cand in accepted:
        ...
        claim = Claim(
            ...
            source_ids=[src.id],
            notes="external claim; not measured by ATHERYN",
...
def _normalize_llm_candidates(raw, passages) -> list[dict]:
    ...
    ATHERYN does not invent provenance to make a claim pass.
    """
    out = []
...
def detect_claim_conflicts(state) -> list[dict]:
    ...
    confidence. Only an ATHERYN experiment can settle a performance question.
    """
    parsed = []
        ...
        for c2, a2, b2, f2 in parsed[i + 1:]:
            ...
            record = {
                ...
                "description": (
                    ...
                    f"SPECULATION; only an ATHERYN experiment can settle it."),
                "ts": time.time()}
