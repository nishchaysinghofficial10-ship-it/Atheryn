"""Durable, bounded autonomy for one ATHERYN mission.

ATHERYN can continue a mission across many short sessions, restarts and pauses.
...
# action types
# Every action maps to an existing, already-gated ATHERYN capability. There is
...
class MissionLease:
    ...
    (`atheryn autonomy recover-lock`) and is audited.
    """
    ...
    def acquire(self, owner: str | None = None) -> Lease:
        ...
        try:
            ...
            raise LeaseHeld(
                ...
                f"are certain it is not, inspect it with 'atheryn autonomy "
                f"status' and release it deliberately with 'atheryn autonomy "
                f"recover-lock --force'. ATHERYN never steals a lease "
...
class AutonomyStore:
    ...
    def load(self) -> None:
        ...
        if version > AUTONOMY_SCHEMA_VERSION:
            raise AutonomyError(
                f"autonomy state schema v{version} is newer than this ATHERYN "
