"""Conservative comparisons for noisy benchmark samples.

ATHERYN measures wall-clock time on whatever machine it happens to run on.
...
Design rules (documented in docs/verification/PERFORMANCE_REPRODUCIBILITY.md):
  ...
  * A separation must also clear a minimum RELATIVE margin (`MIN_REL_MARGIN`),
    ...
    run.
  * No p-values are reported. With the trial counts ATHERYN can afford, a
...
def row_stats(row: dict) -> dict:
    ...
    Schema 2 rows carry per-trial `samples`. Schema 1 rows (ATHERYN <= v1.1)
