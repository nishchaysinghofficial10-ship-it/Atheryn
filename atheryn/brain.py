"""Provider-facing proposal code: a model suggests, and ATHERYN decides.
...
class Brain(ABC):
    ...
    def propose_research(self, context: dict, kinds: tuple = (), k: int = 4) -> list:
        """Return raw structured proposals (see atheryn.proposals).
...
class AnthropicBrain(Brain):
    ...
    def __init__(self, model: str | None = None, timeout_s: float = 60.0,
        ...
        if not self.key:
            raise BrainConfigError(
                "ANTHROPIC_API_KEY is not set. ATHERYN never stores keys; export "
                "the variable in your environment, or use --brain mock.")
        self.model = model or os.environ.get("ATHERYN_BRAIN_MODEL",
                                             "claude-sonnet-4-6")
        ...
        self.audit_raw = os.environ.get("ATHERYN_LLM_AUDIT_RAW", "") == "1"
    ...
    def _audit_raw_exchange(self, purpose, system, user, text) -> None:
        """Opt-in raw audit (ATHERYN_LLM_AUDIT_RAW=1). Off by default: prompts
    ...
    def propose_research(self, context, kinds=(), k=4):
        ...
        Validation is deliberately NOT done here: `atheryn.proposals.review()`
