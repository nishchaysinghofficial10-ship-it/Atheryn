"""Durable mission state, checkpoints, and append-only events.

Everything ATHERYN learns or decides is persisted after every controller step.
...
class ResearchState:
    ...
    def load(cls, root: Path) -> "ResearchState":
        ...
        if not main.exists() and not bak.exists():
            raise FileNotFoundError(
                f"No ATHERYN project at {root} (no state.json or state.json.bak)")
            ...
            try:
                ...
                if sv > SCHEMA_VERSION:
                    raise CheckpointCorrupted(
                        f"Checkpoint schema v{sv} is newer than this ATHERYN "
    ...
    def _from_snapshot(cls, root: Path, d: dict, sv: int) -> "ResearchState":
        ...
        # Portability migration: ATHERYN <= v1.0 stored absolute artifact paths,
