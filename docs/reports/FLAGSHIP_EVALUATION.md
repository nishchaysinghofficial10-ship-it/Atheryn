# ATHERYN — Flagship Autonomous Research Evaluation
...
ATHERYN's machinery actually buys. Pre-registration was written to disk **before
...
ATHERYN was not steered: the hypotheses are the domain's standing templates plus
...
atheryn_full    hypotheses → predictions → budgeted experiments → analysis
...
| | baseline | proposal_only | atheryn_full |
|---|---:|---:|---:|
...
ATHERYN spent six experiments instead of one and produced *fewer* conclusions —
...
### What ATHERYN rejected — using its own evidence
...
Both were ATHERYN's own seeded hypotheses, and both are textbook-plausible. It
...
python3 -m atheryn verify  --dir runs/flagship_eval/atheryn_full
python3 -m atheryn report  --dir runs/flagship_eval/atheryn_full
python3 -m atheryn replay  --dir runs/flagship_eval/atheryn_full --exp <exp_id>
...
results. `atheryn verify` passes; the portability guard reports no absolute paths.
...
3. **The comparison is not blind.** The same author wrote all three workflows.
   ...
   it has to be *added*, and ATHERYN has it by construction.
...
5. **No novelty is claimed.** That SPFA relaxes fewer edges than Bellman-Ford
   and that BFS needs unit weights are textbook facts. ATHERYN rediscovered them
