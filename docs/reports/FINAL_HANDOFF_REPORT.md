# ATHERYN v1.0 — Final Handoff Report

Read this as a handoff note: what ran, what it showed, and what still needs checking.
...
|---|---|---|
| Validated mission lifecycle, legal transitions only | `atheryn/lifecycle.py` | `test_lifecycle.py` (6 tests) incl. illegal-transition rejection, terminal finality, v0.1 migration |
...
| Corrupt-checkpoint recovery + safe failure | `atheryn/state.py` | 2 tests (backup recovery; both-corrupt raises) |
| Experiment replay from recorded metadata within tolerance | `atheryn replay` | test + live run: `exp_f53e0d9748`, 16 cells, PASS |
| State consistency verification | `state.verify()` / `atheryn verify` | flagship: "State verified"; duplicate-event detection test |
| Budgets (experiments, compute, wall time, provider calls, retries) + stop reasons | `atheryn/budget.py` | 3 tests; flagship stop reason recorded |
| Sandbox policy: reject-before-spawn, rlimits, scrubbed env, output caps | `atheryn/sandbox.py`, `experiments.py` | `test_sandbox.py` (6 tests) incl. memory-bomb kill and env scrub |
| LLM proposal layer with schema + vocabulary validation, retries, redaction, budget | `atheryn/brain.py` | `test_brain.py` (7 tests) |
...
| Untrusted evidence ingestion with provenance and dedupe | `atheryn/evidence.py` | 2 tests + live ingest into flagship (injection payload inert) |
...
python -m atheryn init … --profile flagship      → project created
python -m atheryn run --dir examples/flagship_run→ COMPLETED, step 20,
                                                   13 experiments, 80.3 s compute
python -m atheryn verify --dir examples/flagship_run
                                                → "State verified: …consistent."
python -m atheryn replay --dir … --exp exp_f53e0d9748
                                                → REPLAY PASS (16 cells, ±50 %)
python -m atheryn ingest --dir … --file /tmp/sorting_notes.md
                                                → 3 SPECULATION claims, sha256 recorded
python -m atheryn html --dir …                   → reports/mission_control.html
python -m atheryn status/report/timeline --dir … → consistent with stored state
```
...
2. Quick sort (median-of-three) stays within 25 % of the best candidate on
   random input and doesn't collapse on reversed input.
...
4. The ATHERYN-synthesized hybrid (merge sort with an insertion cutoff) beats
...
recorded caution — intentionally **not** promoted.
...
- **P-1 (critical)**: experiment artifacts were referenced by absolute path, so
  ...
  the published `atheryn-v1.0.zip` embedded 51 machine-specific paths. The
...
`atheryn-v1.0.zip` is superseded and should not be redistributed.
