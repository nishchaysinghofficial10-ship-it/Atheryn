# ATHERYN v1.5 — Autonomy Implementation Report

The report favors a traceable result over a polished narrative; follow the linked artifacts when details matter.

Bounded autonomy: ATHERYN can continue a mission across many short sessions,
...
3. **No single-writer protection** — two `atheryn run` processes on one mission
...
```
atheryn/autonomy.py     WorkItem + JSON schema, MissionLease, AutonomyStore,
                       AutonomyPolicy, RunLimits, retry classification, backoff
atheryn/scheduler.py    Scheduler.tick() — one bounded restart-safe action
...
**New:** `atheryn/autonomy.py`, `atheryn/scheduler.py`, `tools/autonomy_demo.py`,
...
**Modified:** `atheryn/cli.py` (new `autonomy` subcommand group; no existing
command or flag changed), `atheryn/state.py` (`verify()` extended to autonomy
state), `atheryn/__init__.py` + `pyproject.toml` (1.5.0), `README.md`,
...
```
atheryn autonomy status  --dir M     lifecycle, queue, lease, next action, budgets,
                                    retry state, interrupted items, stop reason
atheryn autonomy plan    --dir M     candidates in policy order, with <NETWORK> /
                                    <PROVIDER> markers, every rejection + reason,
                                    the choice, the tie-break, the next wake time
atheryn autonomy tick    --dir M     at most one action
atheryn autonomy run     --dir M --max-steps N --max-wall-s N
                                    [--allow-network --max-retrievals N]
                                    [--allow-provider --max-provider-calls N]
                                    [--max-consecutive-failures N]
atheryn autonomy pause|resume|cancel --dir M
atheryn autonomy recover-lock --dir M [--force]
```
...
as FAILED and surfaced by `atheryn verify` rather than executed.
...
network or provider access, or can't be afforded. Every evaluation writes an
...
$ python3 tools/autonomy_demo.py --dir /tmp/atheryn-autonomy-demo     completed
$ python3 -m atheryn verify --dir /tmp/atheryn-autonomy-demo           State verified
$ python3 -m atheryn verify --dir examples/autonomy_demo              State verified
...
2. **The lease is advisory and filesystem-local**; it assumes reliable
   `O_CREAT|O_EXCL` and doesn't span an unreliable network filesystem.
...
to find out which parts of ATHERYN are genuinely domain-neutral and which have
