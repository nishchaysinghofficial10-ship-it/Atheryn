# ATHERYN v1.4.1 — Web Retrieval Security Correction Report

These figures describe one recorded run; repeat the cited command before treating them as current on another machine.
...
1. the default opener installs `HTTPRedirectHandler`, so redirects were
   followed **automatically** — the one thing the document path intentionally
   refuses to do;
...
The file that decides whether ATHERYN may fetch a document was itself being
retrieved outside the rules ATHERYN applies to documents.
...
what changed is that ATHERYN no longer claims to have honoured rules it never
...
vulnerability isn't a bypassed check — it is that `gzip.decompress()` is
...
```
$ python3 -c "import gzip; from atheryn import retrieval as R; \
...
Why it mattered: ATHERYN would have created a source record whose stored
...
validating every returned address, ATHERYN connects to a validated address by
...
$ python3 -m atheryn verify --dir examples/evidence_demo
...
pypi.org robots decision for /simple/: (False, 'disallowed_by_policy')
  refused by live robots.txt: robots.txt at pypi.org disallows this path for ATHERYN-research
live doc: 200 44051B robots=absent pinned=185.199.108.133
```

`pypi.org/robots.txt` disallows `/simple/`; ATHERYN fetches the rules through
...
4. **Robots caching is per-provider-instance and unbounded in lifetime** — a
   long-lived process won't notice a robots.txt that changes mid-run.
