# ATHERYN v1.3 — LLM Proposal Layer Verification Report

Read the numbers as a snapshot of the captured environment, not as a promise about a different host.
...
provider text
  → atheryn/proposals.py :: parse_provider_json    strict JSON, no repair
  ...
  → controller._admit_proposal                    ATHERYN decides what it becomes
  → normal pipeline                               experiment → analysis → critic
                                                  → replication → confidence
```

New modules: `atheryn/proposals.py` (schemas, validation, audit log),
`tools/live_llm_check.py` (bounded live verification). `atheryn/brain.py` gained
...
validator rejects it with an accurate reason instead of ATHERYN crashing.
...
| `experiment` | `statement`, `rationale`, `suggested_experiment{algorithms,regimes,sizes,trials}` | a candidate design; ATHERYN sets seed, timeout, round, coverage |
...
ATHERYN from a content hash; a provider cannot choose its own identity. Unknown
...
NOT RUN — ANTHROPIC_API_KEY is not set. ATHERYN never stores keys; export the
...
returning a realistic Anthropic envelope (three proposals, one intentionally
out-of-vocabulary):
...
dossier §16b reproduces that ledger with what ATHERYN did with each.
...
hypothesis with ATHERYN's seed and timeout (`experiment_proposal_used` event),
...
2. **Real provider behaviours** — genuine 429 bodies, streaming, partial
   responses, `overloaded_error`, and real latency distributions haven't been
