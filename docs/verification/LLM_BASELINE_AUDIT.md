# ATHERYN — LLM Layer Baseline Audit (before v1.3)

The artifact is the useful part of this note. Re-run its command when the surrounding environment changes.

Method: direct inspection of `atheryn/brain.py` (272 lines), its call sites in
...
1. **Proposal model is too thin to evaluate.** No `proposal_id`, assumptions,
   ...
   limitations — so ATHERYN can't reason about a proposal's cost/benefit, and a
   rejected proposal can't be audited after the fact.
2. **Only one proposal type is usable.** Experiment designs, counterarguments
   and knowledge gaps are named in the specification but don't exist in code.
...
7. **Experiment proposals have no policy gate of their own.** Today an LLM
   can't propose an experiment at all; when it can, that path must run through
