# ATHERYN — Performance Reproducibility

Read the numbers as a snapshot of the captured environment, not as a promise about a different host.

What ATHERYN promises about an experiment it re-runs, and — just as important —
...
TIER 3 — ABSOLUTE TIMING VALUES                     never asserted
  ...
  pass/fail criterion, because ATHERYN cannot control the host it runs on.
```

The separation exists because ATHERYN's own measurements proved it necessary:
...
**Backward compatibility.** Schema-1 results (ATHERYN ≤ v1.1) have no samples,
...
Implemented in `atheryn/stats.py`. Every gate must pass:
...
would communicate a precision the data doesn't contain. ATHERYN reports the
...
```bash
python -m atheryn replay --dir <mission> --exp <exp_id>            # default
python -m atheryn replay --dir <mission> --exp <exp_id> --strict   # quiet host
```
...
## 5. How this changes what ATHERYN concludes
