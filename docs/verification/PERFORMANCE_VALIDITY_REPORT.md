# ATHERYN v1.2 — Performance Validity Report

The artifact is the useful part of this note. Re-run its command when the surrounding environment changes.

Engagement: make ATHERYN's performance measurement and experiment replay
...
**One core, shared virtualised host.** That isn't an ideal benchmarking
...
1. **Result schema v2** (`atheryn/domains/algobench.py` runner template): per-trial
...
2. **`atheryn/stats.py`** — conservative comparison rules (≥5 trials both sides,
   separation > 3×(SEM+SEM), margin ≥ 10 %), plus `indistinguishable_set()` for
   reporting ties honestly. No p-values.
3. **`atheryn/replay.py`** — the comparison engine, extracted from the CLI so it
...
5. **Sweeps report tie sets** — the optimum plus every setting statistically
   indistinguishable from it; a sweep prediction whose interval can't be
...
```
$ python3 -m atheryn replay --dir /tmp/ctl/wrong --exp exp_a7115d9073 \
...
```
$ python3 -m atheryn replay --dir /tmp/ctl/tamper --exp exp_a7115d9073
...
```
$ python3 -m atheryn replay --dir /tmp/ctl/noise --exp exp_a7115d9073
...
$ python3 -m atheryn replay --dir /tmp/ctl/noise --exp exp_a7115d9073 --strict
exit=1
```
...
```
$ python3 -m atheryn replay --dir /tmp/ctl/invert --exp exp_819300a3a7
...
$ python3 -m atheryn replay --dir /tmp/ctl/invert --exp exp_819300a3a7 --strict
...
```
$ python3 -m atheryn init "Under what input distributions and sizes does a hybrid
...
$ python3 -m atheryn run --dir examples/flagship_run
...
$ python3 -m atheryn replay --dir examples/flagship_run --exp exp_a7115d9073
...
```
$ python3 -m atheryn replay --dir examples/demo_run --exp exp_defeaebae2
...
6. **Cross-machine reproduction hasn't been performed.** Everything in this
