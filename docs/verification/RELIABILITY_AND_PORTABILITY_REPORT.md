# ATHERYN — Reliability & Portability Verification Report

The artifact is the useful part of this note. Re-run its command when the surrounding environment changes.
...
- Repository at start of engagement: `f6b46e8` (ATHERYN v1.0 as handed off).
- Repository at end: `atheryn` package version **1.1.0**, 56 tests.
...
```
$ cp -r /home/claude/atheryn-project /tmp/cleanrun/repo && cd /tmp/cleanrun/repo
...
the time): OK — no deprecation warnings from ATHERYN's own code paths,
...
**Not supported:** Windows. ATHERYN requires `resource` rlimits and
...
exp_aa415f5a05 -> /home/claude/atheryn-project/examples/flagship_run/experiments/exp_aa415f5a05
```
...
$ python3 -m atheryn verify --dir /tmp/portcheck/relocated_mission
...
```
$ unzip -q atheryn-v1.0.zip && grep -rl "/home/claude" .
./atheryn-project/examples/demo_run/state.json
./atheryn-project/examples/demo_run/research_state/experiments.json
./atheryn-project/examples/flagship_run/state.json.bak
./atheryn-project/examples/flagship_run/state.json
./atheryn-project/examples/flagship_run/research_state/experiments.json
...
$ python3 -m atheryn verify --dir /tmp/ci/relocated_mission
State verified: counts, references, experiment artifacts and event log are consistent.

$ python3 -m atheryn replay --dir /tmp/ci/relocated_mission --exp exp_aa415f5a05
...
$ python3 -m atheryn verify --dir /tmp/ci/gutted ; echo exit=$?
...
```
$ git archive --format=zip --prefix=atheryn-project/ HEAD > /tmp/ciarch/atheryn.zip
$ unzip -q /tmp/ciarch/atheryn.zip -d /tmp/ciarch/extracted
$ cd /tmp/ciarch/extracted/atheryn-project
$ python3 tools/check_artifacts_portable.py .
PORTABILITY OK: no machine-specific absolute paths in artifacts under .
$ python3 -m unittest discover -s tests
...
State verified: …
$ python3 -m atheryn replay --dir examples/flagship_run --exp exp_f53e0d9748
REPLAY PASS — …
```
...
```
$ for i in 1 2 3; do python3 -m atheryn replay --dir … --exp exp_aa415f5a05; done
...
**Honest limitation:** on this host ATHERYN can certify that a stored experiment
...
before:  $ printf '{"ts": 1786, "kind": "partial"' >> logs/events.jsonl
         $ python3 -m atheryn timeline --dir …   -> exit=1 (traceback)
         $ python3 -m atheryn verify  --dir …
           - event log unreadable: Expecting property name…
           - event log is empty                 # false: 60+ events were readable
after:   $ python3 -m atheryn timeline --dir …   -> exit=0
         $ python3 -m atheryn verify  --dir …
...
```
$ python3 -m atheryn run --dir /tmp/kill/m &   # standard profile
$ sleep 4 && kill -9 $PID
$ python3 -c "…"
...
```
$ python3 -m atheryn init "hard kill recovery evidence" --dir /tmp/kill/m \
      --profile standard --max-experiments 12 --brain none
$ python3 -m atheryn run --dir /tmp/kill/m &  ; sleep 4 ; kill -9 $PID
  after kill:   phase = CRITICIZING | step = 8  | experiments = 4
$ python3 -m atheryn run --dir /tmp/kill/m
...
$ python3 -m atheryn verify --dir …          # exit 2
...
```
$ python3 -m atheryn verify --dir /tmp/cifresh/ci_mission
State verified: …
$ python3 tools/check_artifacts_portable.py /tmp/cifresh/ci_mission
PORTABILITY OK: …
$ python3 -m atheryn run --dir /tmp/cifresh/ci_resume --steps 2
  phase after --steps 2: PAUSED
$ python3 -m atheryn run --dir /tmp/cifresh/ci_resume
...
6. **Reconciled orphans are not analysed.** Their artifacts are preserved and
   marked `interrupted`; the compute they consumed isn't retroactively charged
   to the budget (the duration was never recorded).
7. **`atheryn-v1.0.zip` as previously published is defective** (P-1). It is
...
$ python3 -m atheryn verify --dir examples/flagship_run
State verified: counts, references, experiment artifacts and event log are consistent.
$ python3 -m atheryn verify --dir examples/demo_run
State verified: counts, references, experiment artifacts and event log are consistent.

$ cp -r examples/flagship_run /tmp/final/relocated
$ python3 -m atheryn verify --dir /tmp/final/relocated
State verified: counts, references, experiment artifacts and event log are consistent.
$ python3 -m atheryn replay --dir /tmp/final/relocated --exp exp_f53e0d9748
...
$ python3 -m atheryn verify --dir /tmp/final/gutted ; echo $?
26 consistency problem(s):
1                       # no phantom pass: the copy cannot borrow another mission's files

$ git archive --format=zip --prefix=atheryn-project/ HEAD > /tmp/final/arch/atheryn.zip
$ cd /tmp/final/arch && unzip -q atheryn.zip && cd atheryn-project
$ python3 tools/check_artifacts_portable.py .
PORTABILITY OK: no machine-specific absolute paths in artifacts under .
$ python3 -m unittest discover -s tests
...
$ python3 -m atheryn verify --dir examples/flagship_run
State verified: counts, references, experiment artifacts and event log are consistent.
$ python3 -m atheryn replay --dir examples/flagship_run --exp exp_aa415f5a05
