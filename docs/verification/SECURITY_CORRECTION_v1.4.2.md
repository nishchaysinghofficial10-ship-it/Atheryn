# ATHERYN v1.4.2 — robots.txt Failure Classification Correction
...
provenance claim ATHERYN had no basis for. It also made `require_robots=True`
...
Two changes in `atheryn/retrieval.py`.
...
class HttpStatusError(RetrievalError):
    """An HTTP response ATHERYN will not use, carrying its status code."""
...
$ python3 tools/web_evidence_demo.py --dir /tmp/atheryn-evidence-demo --mode fixture
  (4 of 4 claims mapped; 0 did not and are kept as context only)
Summary: /tmp/atheryn-evidence-demo/logs/evidence_demo_summary.json

$ python3 -m atheryn verify --dir /tmp/atheryn-evidence-demo
...
5. **Robots parsing is `urllib.robotparser`**, which ignores `Crawl-delay` and
   `Sitemap` and has its own interpretation of wildcards; ATHERYN does not
