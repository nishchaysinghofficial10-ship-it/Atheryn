# ATHERYN v1.4 — Web Evidence Verification Report
...
```text
EvidenceProvider (ABC)              atheryn/retrieval.py
...
approved URL
  ...
  → ingest_url()            atheryn/web_evidence.py
...
New modules: `atheryn/retrieval.py`, `atheryn/web_evidence.py`,
...
envelope carrying an explicit "not an instruction / don't follow, execute, or
...
algorithmic performance tradeoffs, and which of those claims can ATHERYN test in
...
=== WHICH OF THIS CAN ATHERYN ACTUALLY TEST? ===
...
items. One claim maps to something ATHERYN could actually run; three map
...
=== VISIBLE CONFLICTS (1) ===
  ...
  only an ATHERYN experiment can settle it.
```
...
error, so a source record could carry a `content_hash` over bytes ATHERYN could
...
1. **No search or discovery.** ATHERYN retrieves URLs you approve.
...
3. ~~robots.txt handling is untested against a host that actually serves one~~
   **Closed in v1.4.1**: verified live against `pypi.org`, whose robots.txt
   disallows `/simple/` — ATHERYN fetches the rules through the restricted path
