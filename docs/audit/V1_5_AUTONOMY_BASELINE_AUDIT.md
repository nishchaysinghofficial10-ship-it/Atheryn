# ATHERYN — v1.5 Autonomy Baseline Audit

Treat this as a maintainer's notebook: observations first, conclusions only where the evidence supports them.
...
$ python3 -m atheryn verify --dir examples/flagship_run → State verified
$ python3 -VV → Python 3.12.3 [GCC 13.3.0], Ubuntu 24.04.4 LTS, x86_64
```

## Existing state model (`atheryn/state.py`, SCHEMA_VERSION 3)
...
## Lifecycle (`atheryn/lifecycle.py`)
...
## Budget enforcement points (`atheryn/budget.py`)
...
| Evidence atheryn | only experiments create `Evidence` | unchanged |
| Replication + falsification | `CriticEngine` | unchanged |
| State integrity | `state.verify()` | extended, not bypassed |

## Concurrency

**None today.** Two `python -m atheryn run --dir X` processes would both load,
