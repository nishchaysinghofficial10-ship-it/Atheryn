...
These are working audit notes, so unresolved questions and rough edges remain visible.
...
|---|---|---|---|---|---|---|---|---|
| Persistent research state (atomic save, typed reload, per-type views, event log) | yes | yes | yes | real | `atheryn/state.py` | IMPLEMENTED_AND_VERIFIED | `test_roundtrip_and_resume` ok; demo `state.json` reloads | Atomic tmp+rename; append-only `events.jsonl` |
| Mission/controller state machine | partial | yes | partial | real | `atheryn/controller.py` | PARTIALLY_IMPLEMENTED | e2e test reaches `complete` | Phases are loose strings; **no transition validation**, no FAILED/CANCELLED, no formal PAUSED |
| Competing hypotheses + ledgers | yes | yes | yes | real | `atheryn/models.py`, domain | IMPLEMENTED_AND_VERIFIED | 5 hypotheses in demo; e2e asserts pool ≥5 | 4 base + 1 evolved |
...
| "Sandboxed" experiment execution | partial | yes | partial | real | `atheryn/experiments.py` | PARTIALLY_IMPLEMENTED | 6 demo experiments ran; failure path exercised | Subprocess + timeout ONLY. **No rlimits, env not scrubbed, no output caps, network not blocked.** "Sandbox" is currently an overclaim |
...
| Resource budgets | partial | yes | yes | real | `atheryn/budget.py` | PARTIALLY_IMPLEMENTED | `test_accounting_and_exhaustion`, `test_budget_stops_research` | Experiments+compute only. **No elapsed-time, retry, or provider-call budgets; no stop-reason reporting** |
| Adversarial critic | partial | yes | partial | real | `atheryn/critic.py` | PARTIALLY_IMPLEMENTED | demo: replication forced, cautions emitted | Replication + assumption audit + contradiction surfacing exist. **No falsification experiments, no boundary probes, no alternative-hypothesis attack** |
...
| Knowledge graph + contradiction detection | yes | yes | yes | real | `atheryn/graph.py` | IMPLEMENTED_AND_VERIFIED | `test_contradiction_detection`; demo flagged 2 real conflicts | No conditions/temporal edges; no visualization |
...
| Reports: dossier + timeline + status box | yes | yes | yes | real | `atheryn/report.py` | IMPLEMENTED_AND_VERIFIED | files in `examples/demo_run/reports/` | No prediction ledger table, no threats-to-validity section, no HTML |
