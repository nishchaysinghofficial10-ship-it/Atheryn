...
| 5 | Replay prior experiment from metadata within tolerance | PARTIALLY (manual only) | **BUILD**: `atheryn replay` command + test |
...
| 11 | LLM provider abstraction, env credentials, schema-validated structured output, retries/timeouts, mock provider, injection-aware, no LLM→fact path | MISSING | **BUILD**: `atheryn/brain.py` (Mock + Anthropic via stdlib urllib); proposals-only pathway |
