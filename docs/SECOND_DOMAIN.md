...
This is the design note for `graphbench`, including the places where a second domain forced the core to change.
...
`atheryn/domains/graphbench.py` — one file, implementing only the
...
- **Baselines:** `dijkstra_heap`, `dijkstra_array` (O(V²) scan),
  ...
  graphs, included intentionally so the domain contains a real correctness
