# ATHERYN — Evidence Acquisition

Retrieved material enters as untrusted context and keeps its provenance all the way through the pipeline.

ATHERYN can retrieve documents. It does not browse, crawl, agentically navigate,
or obey them. This note is the retrieval policy, the provenance model, and
the exact commands.
...
approved URL → policy-checked retrieval → stored source record (provenance)
  ...
  → an ATHERYN experiment, if the claim is testable, produces the finding
```
...
| User agent | `ATHERYN-research/1.4 (…)` | identifies the tool and its purpose |
...
| Downloaded code | never executed | ATHERYN only ever runs code generated from its own audited domain templates |
| Access controls | never bypassed | no logins, no cookies, no paywall circumvention, no scraping of restricted areas |

Policy lives in code (`atheryn/retrieval.py`). Nothing in a retrieved document
can change it.
...
| `unavailable` | ATHERYN could not find out | timeout, DNS/connection/TLS failure, HTTP 5xx, any non-404 status, redirect-policy refusal, redirect over the cap, oversized body, malformed or truncated compressed body, unsupported encoding, undecodable content |
...
cannot be located is rejected, and ATHERYN does not invent provenance to rescue
it.
...
is the provenance — so do not point ATHERYN at pages containing secrets.
...
python -m atheryn ingest --dir runs/m --url https://example.org/notes \
    --allow-host example.org --max-requests 5
python -m atheryn ingest --dir runs/m --file local_notes.md      # unchanged
python -m atheryn ingest --dir runs/m --provider fixture \
...
- Retrieve only sources you are allowed to retrieve. ATHERYN honours robots.txt
  and does not bypass access controls; the operator is still responsible for
  the terms of each site.
- `license_note` records that ATHERYN has **not** verified licensing. Check the
...
1. **No search.** ATHERYN retrieves URLs you approve; it doesn't discover them.
...
5. **The testability mapping in the demo is a keyword heuristic**, not
   comprehension. It intentionally reports "not measurable" and "unmapped"
...
6. **General-web retrieval is partly unverified in the build environment**:
   ...
   *was* verified end to end (pypi.org disallows `/simple/`, and ATHERYN
   refuses it).
7. **DNS rebinding is mitigated, not eliminated.** ATHERYN validates every
