# ATHERYN — LLM Integration

A model may suggest work, but it never gets to turn its own prose into evidence.

ATHERYN can use a language model as a **proposal generator**. It never uses one
as an authority. This note is what a new developer needs to run the mock
...
| `experiment` | a *candidate design*; ATHERYN sets seed, timeout, round and scope, and the sandbox policy gate still applies | an execution instruction |
...
critic, replication and falsification as one ATHERYN generated itself, and it
...
python -m atheryn init "…" --dir runs/m --brain mock

# no proposals at all
python -m atheryn init "…" --dir runs/m --brain none
...
| `ATHERYN_BRAIN_MODEL` | model name | defaults to `claude-sonnet-4-6` |
| `ATHERYN_LLM_AUDIT_RAW` | set to `1` to store raw prompts/responses | **off by default**; output is redacted; see §6 |
...
The engine depends on `Brain` and on `atheryn/proposals.py`, never on a vendor
...
| `expected_information_gain` | optional | 0.0–1.0 → drives ATHERYN's prioritisation |
...
`proposal_id` is derived by ATHERYN from a hash of the content; a provider
can't choose its own identity. Unknown fields are **rejected**, not ignored —
...
provider text
  ...
  → ATHERYN decides   admitted objects enter the normal pipeline
```
...
Prompt and response bodies aren't written here.

`logs/proposals.jsonl` — every proposal ATHERYN was offered, accepted or
rejected, with its body, its validation stage and the reason. This is the file
to read when asking "what did the model want, and why did ATHERYN refuse?".
Bodies pass through the secret redactor before being written.

`ATHERYN_LLM_AUDIT_RAW=1` additionally writes `logs/brain_raw_audit.jsonl` with
...
## 10. What ATHERYN does not trust from a model
...
- Algorithm names, regime names, metric names, or check kinds outside the
  domain vocabulary.
- Experiment parameters: seed, timeout, round and scope are always ATHERYN's.
