# ATHERYN v1.5 — Reproducibility

Reproduction starts with recorded inputs and code; matching wall-clock numbers is a separate question.
...
```bash
unzip atheryn-v1.4.zip && cd atheryn-project
python -m unittest discover -s tests -v          # 186 tests, ~65 s
python -m atheryn init "demo question" --dir runs/demo --profile fast
python -m atheryn run --dir runs/demo
python -m atheryn status --dir runs/demo
python -m atheryn report --dir runs/demo
python -m atheryn html --dir runs/demo            # reports/mission_control.html
```

## Reproduce the flagship mission exactly
```bash
python -m atheryn init "Under what input distributions and sizes does a hybrid \
...
python -m atheryn run --dir runs/flagship
```
...
python -m atheryn verify --dir /tmp/relocated_mission
python -m atheryn replay --dir /tmp/relocated_mission --exp exp_aa415f5a05
```
...
P-1 — see the verification report). Projects written by ATHERYN ≤ 1.0 with
...
```bash
python -m atheryn replay --dir runs/flagship --exp exp_xxxx \
...
```bash
python -m atheryn run --dir runs/m --steps 2     # stop early -> PAUSED
python -m atheryn run --dir runs/m               # resume (CLI or library API)
python -m atheryn verify --dir runs/m            # always safe to run afterwards
```
...
python - <<'PY'
import json
from atheryn.state import ResearchState
...
**What isn't reproducible:** the content of a live page. Web documents change
and disappear; the stored hash and cached text are what ATHERYN can guarantee
...
python -m atheryn verify --dir runs/autonomy_demo
...
```bash
python -m atheryn autonomy status --dir runs/m     # queue, lease, budgets, stop reason
...
4. `state.json` → `confidence_history` — every status/confidence change with a
   reason.
5. `python -m atheryn verify --dir …` — cross-checks references, on-disk
