...
A release is ready only when the checklist, tests, and distributable tell the same story.
...
| `atheryn init` + `run` (fast profile) | COMPLETED, stop reason "no high-value next experiment remained" |
| `atheryn report \| head -3` | **DEFECT: `BrokenPipeError` traceback** |
| dossier header | **DEFECT: hardcoded "ATHERYN v1.0"** — stale for five releases |
| `atheryn verify --dir examples/flagship_run` | State verified |
| `atheryn replay --dir examples/flagship_run --exp …` | REPLAY PASS |
...
$ python3 -m atheryn run --dir runs/pr --steps 2      → Research status: PAUSED
$ python3 -m atheryn run --dir runs/pr                → COMPLETED, verify clean
$ python3 -m atheryn init --domain graphbench --profile graph_fast … && run
                                                     → State verified
...
```
$ git archive --format=zip --prefix=atheryn/ HEAD > /tmp/cleanroom/atheryn-release.zip
$ cd /tmp/cleanroom && unzip -q atheryn-release.zip && cd atheryn
$ python3 tools/check_artifacts_portable.py .
PORTABILITY OK: no machine-specific absolute paths in artifacts under .
$ python3 -c "import atheryn; print(atheryn.__version__)"
2.0.0
$ python3 -m unittest discover -s tests
...
examples/final_flagship_mission/atheryn_full/     State verified
...
`atheryn verify` on it correctly reports "NO PROJECT". A README in that directory
