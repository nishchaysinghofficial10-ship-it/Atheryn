# ATHERYN — Engineering Decision Log

This log keeps the reasons behind design choices, including the alternatives that were left on the table.
...
benchmark evaluator can't judge produced "inconclusive" verdicts that were
...
2026-08-09. `atheryn/schema.py` (~50 lines) covers type/required/enum/bounds/
...
2026-08-09. `atheryn html` renders `reports/mission_control.html` from stored
...
lacks. **Tradeoff**: intentionally biased toward type II errors. Real but small
...
passage is not present in the retrieved document. ATHERYN does not repair
...
source can never look as solid as ATHERYN's own measurement, however well hosted.
...
cap — so the file that decides whether ATHERYN may fetch a document was itself
retrieved outside the rules ATHERYN applies to documents. **Fix**: a single
...
absence is unchanged — what changed is that ATHERYN stops claiming it obeyed
...
rebinding race. ATHERYN now connects to a validated address while keeping SNI
...
guarantee, ATHERYN falls back and stores `pinned_address` — empty means the race
...
the outcome is `unavailable`. A document ATHERYN can't read isn't a document
...
repeating a side effect ATHERYN doesn't understand, and a safety refusal
