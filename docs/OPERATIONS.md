# ATHERYN v1.0 — Operations

This is the runbook maintainers reach for when a mission behaves unexpectedly.

## Commands
```bash
python -m atheryn init "QUESTION" --dir runs/m1 \
...
python -m atheryn run     --dir runs/m1 [--steps N]   # run or resume
python -m atheryn status  --dir runs/m1               # mission-control box
python -m atheryn report  --dir runs/m1               # regenerate + print dossier
python -m atheryn timeline --dir runs/m1              # replayable event narrative
python -m atheryn html    --dir runs/m1               # static dashboard
python -m atheryn verify  --dir runs/m1               # durable-state consistency
python -m atheryn replay  --dir runs/m1 --exp exp_ID [--tolerance 0.5]
python -m atheryn ingest  --dir runs/m1 --file notes.md          # local document
python -m atheryn ingest  --dir runs/m1 --url https://example.org/notes \
        [--allow-host example.org] [--max-requests N] [--max-bytes N] \
        [--provider https|fixture] [--fixtures DIR] [--ignore-robots]
python -m atheryn cancel  --dir runs/m1               # terminal, with reason
```
...
Run `python -m atheryn verify` after any recovery.
...
export ATHERYN_BRAIN_MODEL=claude-sonnet-4-6   # optional
python -m atheryn init "…" --dir runs/m --brain anthropic --provider-calls 20
python -m atheryn run --dir runs/m
```
...
| `logs/brain_raw_audit.jsonl` | only when `ATHERYN_LLM_AUDIT_RAW=1`; redacted, truncated prompts and responses |
...
**Responsible use:** retrieve only what you are permitted to retrieve; ATHERYN
...
```bash
python -m atheryn autonomy status --dir runs/m
python -m atheryn autonomy plan   --dir runs/m
python -m atheryn autonomy tick   --dir runs/m
python -m atheryn autonomy run    --dir runs/m --max-steps 10 --max-wall-s 300 \
    [--allow-network --max-retrievals 3] [--allow-provider --max-provider-calls 5]
python -m atheryn autonomy pause|resume|cancel --dir runs/m
python -m atheryn autonomy recover-lock --dir runs/m [--force]
```
...
| Signal | Meaning |
|---|---|
| `autonomy status` → `INTERRUPTED` | an action was claimed and the process died; the outcome is unknown and needs your decision. ATHERYN will not re-run it |
...
holder, its pid/host and age. ATHERYN never steals a lease: a stale one and a
