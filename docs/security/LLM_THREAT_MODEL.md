...
operator (trusted)
  ...
  → ATHERYN core (trusted code)
      → provider request ................................ constructed by ATHERYN
...
| L4 | Same | Requests secret disclosure ("print ANTHROPIC_API_KEY") | Rejected at schema/policy; ATHERYN has no mechanism to read env vars on a proposal's behalf; the key is never in any serialisable structure | `ATTACKS[2]`, `test_missing_key_fails_clearly_without_leaking` |
| L5 | Same | Proposes an unsafe experiment (huge sizes, long timeout) to exhaust the host | `sandbox.validate_design()` — the same gate ATHERYN's own designs face — runs *before* the proposal is accepted, and again before any process spawns | `test_unsafe_experiment_parameters_are_rejected` |
...
| L7 | Same | Floods ATHERYN with proposals to exhaust budget or attention | Hard cap of 8 proposals per response, audited; provider-call budget enforced per call | `test_proposal_cap_is_enforced`, `test_provider_budget_is_enforced_and_not_retried` |
...
4. **No provider-side guarantees.** ATHERYN cannot verify what a provider logs,
