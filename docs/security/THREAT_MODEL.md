# ATHERYN v1.0 — Threat Model

The useful distinction here is between a control the code enforces and a protection supplied by the host.

## Assets
1. **Research integrity** — hypotheses, evidence, verdicts, confidence history.
   Corrupting these is the highest-value attack: it makes ATHERYN lie credibly.
...
| T3 | Author of an ingested document | Prompt injection ("ignore previous instructions…") | Document content is wrapped in an explicit untrusted-data envelope, claims are capped at confidence 0.4 and SPECULATION status, and ATHERYN takes no action from document text. Tested: `test_evidence_redteam.py::test_ingested_content_is_untrusted_speculation_with_provenance` |
...
| T6 | Local attacker with FS access | Corrupt/poison the checkpoint | Backup rotation + safe load + explicit `CheckpointCorrupted`; `atheryn verify` cross-checks references, on-disk artifacts, and duplicate events. ATHERYN does **not** cryptographically sign state — an attacker with write access to the project directory can still forge history (residual risk) |
...
| T14 | ATHERYN itself | Publish a conclusion the stored state does not support | Report-vs-truth tests: accepted claims must exist in state with scope and a `replicated` tag; incorrect candidates cannot be called fastest; universalising phrases are banned | RT-45…RT-47 |
...
- Multi-tenant/hostile-operator use. ATHERYN assumes the operator is trusted.
