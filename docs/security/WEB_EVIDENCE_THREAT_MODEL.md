# ATHERYN — Web Evidence Threat Model

This is a boundary review, not a claim of perfect isolation; each remaining gap stays beside the control that mitigates it.
...
operator (trusted)
  ...
  → ATHERYN core (trusted code)
      → HTTP request ............................... constructed by ATHERYN
...
| W4 | Attacker controlling a URL ATHERYN is given | SSRF to `169.254.169.254`, `127.0.0.1`, RFC1918, or `localhost` | Every resolved address checked against loopback/private/link-local/multicast/reserved | `test_loopback_private_and_metadata_addresses_are_rejected` |
| W5 | Same | DNS rebinding: a public name resolving to a private address | *All* addresses from `getaddrinfo` are checked, not just the first; the connection is then **pinned** to a validated address (SNI/cert still bound to the hostname), closing the resolve-then-connect race for the connection ATHERYN makes. See residual risk 6 for what pinning does not cover | `test_hostname_resolving_to_private_address_is_rejected`; live check records `pinned_address` |
...
| W9 | Same | Publishes a plausible false performance claim | Claims enter as SPECULATION ≤0.4 with a stored passage; in the algorithms domain, findings come only from ATHERYN experiments | `test_no_web_claim_becomes_accepted_knowledge` |
...
| W12 | Model doing extraction | Fabricates a claim with no supporting passage | Passage must be locatable in the retrieved text or the claim is rejected; ATHERYN never invents provenance | `test_llm_candidates_without_provenance_are_rejected_not_repaired` |
...
| W14 | Operator error | Points ATHERYN at a private URL or a huge crawl | Policy refuses loudly (raises) rather than silently skipping; budgets and allow-lists are explicit | `test_cli_refuses_a_policy_violating_url` |
...
3. **robots.txt is honoured but terms of service aren't parsed.** The operator
...
6. **DNS rebinding: mitigated, with a named gap.** ATHERYN resolves a host,
   ...
   - if pinning can't be established (interpreter internals unavailable, or a
   ...
   - only the first validated address is used, so ATHERYN does not fail over to
     a second address;
...
8. **General-web retrieval is partly unverified here.** Egress in the build
   ...
   verified (pypi.org disallows `/simple/`; ATHERYN refuses it). Arbitrary
