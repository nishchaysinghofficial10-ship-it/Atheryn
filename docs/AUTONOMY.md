# ATHERYN — Bounded Autonomy

Autonomy here means bounded continuation of known actions, not permission to invent new ones.

ATHERYN can continue a research mission across many short sessions, restarts and
pauses. "Autonomous" here means *ATHERYN chooses which permitted action happens
next*. It does not mean unrestricted, self-modifying, or unsupervised.

---

## 1. What ATHERYN may decide on its own
...
- **Re-running an interrupted item** — ATHERYN marks it `interrupted` and stops
...
```bash
python -m atheryn autonomy status  --dir runs/m     # queue, lease, budgets, next action
python -m atheryn autonomy plan    --dir runs/m     # candidates + why the next one wins
python -m atheryn autonomy tick    --dir runs/m     # at most ONE action
python -m atheryn autonomy run     --dir runs/m --max-steps 10 --max-wall-s 300
python -m atheryn autonomy pause   --dir runs/m     # durable, at the next checkpoint
python -m atheryn autonomy resume  --dir runs/m
python -m atheryn autonomy cancel  --dir runs/m     # terminal, durable
...
python -m atheryn autonomy run --dir runs/m --allow-network --max-retrievals 3
python -m atheryn autonomy run --dir runs/m --allow-provider --max-provider-calls 5
```
...
truthful stop reasons; `atheryn verify` covers autonomy state.

**Not guaranteed:** exactly-once execution of an *external* action — if a
process dies mid-action, ATHERYN records the ambiguity rather than claiming a
guarantee it can't provide. Also not implemented: multi-agent coordination,
