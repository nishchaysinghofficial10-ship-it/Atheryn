#!/usr/bin/env python3
"""Run the three pre-registered flagship workflows side by side.
...
measures what each produces:
    ...
    proposal    the LLM proposal layer generates hypotheses, and they are
                ...
                produces.
    atheryn      the full loop: competing hypotheses → machine-checkable
                predictions → budgeted experiment selection → analysis →
                criticism → falsification → independent replication → scoped
                conclusions.

The point is not to make ATHERYN look good. It is to measure what the extra
...
from atheryn import stats as st_                               # noqa: E402
from atheryn.brain import MockBrain                            # noqa: E402
from atheryn.budget import Budget                              # noqa: E402
from atheryn.cli import PROFILES                               # noqa: E402
from atheryn.controller import ResearchController              # noqa: E402
from atheryn.domains.base import get_domain                    # noqa: E402
from atheryn.experiments import ExperimentEngine               # noqa: E402
from atheryn.models import HypothesisStatus                    # noqa: E402
from atheryn.state import ResearchState                        # noqa: E402
...
PREREGISTRATION = {
    ...
    "note": ("Registered before any workflow was run. ATHERYN was not steered "
...
def run_proposal_only(root: Path) -> dict:
    ...
    from atheryn import proposals as P
...
# workflow C
def run_atheryn(root: Path) -> dict:
    """The full research loop."""
    st = _mission(root, "atheryn_full", brain="mock")
    t0 = time.time()
    ...
    return {
        "workflow": "atheryn_full", "experiments": st.budget.experiments_used,
        "wall_s": round(time.time() - t0, 1),
